    ...
    <div className="space-y-2">
        ...
        {team.roster.map(player => (
            <div
                ...
                className={`p-2 rounded-lg flex items-center space-x-4 cursor-pointer transition-all duration-200 ${selectedPlayerId === player.id
...
const GameSetup: React.FC<{
    ...
    const handleStart = () => {
        ...
        onStartGame({
            periodType,
            clockType,
            periodLength: isQuarters ? 12 * 60 : 20 * 60,
            totalPeriods: isQuarters ? 4 : 2,
        });
    };

    return (
        <div className="max-w-2xl mx-auto bg-gray-800 p-8 rounded-lg shadow-2xl text-center space-y-8">
            <h1 className="text-3xl font-bold text-cyan-400">Game Setup</h1>
            <p className="text-gray-400">Configure the game settings before you begin.</p>

            <div className="space-y-6">
                <div>
                    <h3 className="text-lg font-semibold mb-2">Period Type</h3>
                    <div className="flex justify-center gap-4">
                        <button onClick={() => setPeriodType('quarters')} className={`px-6 py-3 rounded-lg font-semibold ${periodType === 'quarters' ? 'bg-cyan-600 ring-2 ring-cyan-400' : 'bg-gray-700'}`}>12 min Quarters</button>
                        <button onClick={() => setPeriodType('halves')} className={`px-6 py-3 rounded-lg font-semibold ${periodType === 'halves' ? 'bg-cyan-600 ring-2 ring-cyan-400' : 'bg-gray-700'}`}>20 min Halves</button>
                    </div>
                </div>
                <div>
                    <h3 className="text-lg font-semibold mb-2">Clock Type</h3>
                    <div className="flex justify-center gap-4">
                        <button onClick={() => setClockType('stop')} className={`px-6 py-3 rounded-lg font-semibold ${clockType === 'stop' ? 'bg-cyan-600 ring-2 ring-cyan-400' : 'bg-gray-700'}`}>Stop Time</button>
                        <button onClick={() => setClockType('running')} className={`px-6 py-3 rounded-lg font-semibold ${clockType === 'running' ? 'bg-cyan-600 ring-2 ring-cyan-400' : 'bg-gray-700'}`}>Running Clock</button>
                    </div>
                    <p className="text-xs text-gray-500 mt-2">A running clock is often used in tournaments and does not stop for whistles.</p>
                </div>
            </div>

            <div className="flex gap-4 justify-center pt-6 border-t border-gray-700">
                <button onClick={handleStart} className="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-8 rounded-md text-lg transition-colors">Begin Game</button>
                <button onClick={onReturnToDashboard} className="bg-gray-600 hover:bg-gray-500 text-white font-semibold py-3 px-6 rounded-lg transition-colors">Cancel</button>
            </div>
        </div>
    );
};
...
const LiveStatsSummary: React.FC<{
    ...
    return (
        <div className="bg-gray-800 p-4 rounded-lg shadow-lg">
            <h3 className="text-xl font-bold mb-2 text-center text-cyan-400">Live Stats Summary</h3>
            <table className="w-full text-center">
                <thead>
                    <tr className="border-b-2 border-gray-700">
                        <th className="py-2 font-semibold">{homeTeam.name}</th>
                        <th className="py-2 text-gray-400 text-sm">Stat</th>
                        <th className="py-2 font-semibold">{awayTeam.name}</th>
                    </tr>
                </thead>
                <tbody>
                    {statsToDisplay.map(({ key, label }) => (
                        <tr key={key} className="border-b border-gray-700">
                            <td className="py-2 text-2xl font-mono">{homeTotals[key] || 0}</td>
                            <td className="py-2 text-xs uppercase tracking-wider text-gray-400">{label}</td>
                            <td className="py-2 text-2xl font-mono">{awayTotals[key] || 0}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
...
const GameTracker: React.FC<GameTrackerProps> = ({ game, onUpdateGame, onReturnToDashboard, currentUser, onViewReport }) => {
    ...
    useEffect(() => {
        // This effect synchronizes the local clock state with the global game state.
        // It only triggers an update if the clock value has actually changed,
        // preventing an infinite loop and ensuring we don't send redundant updates.
        if (game.gameClock !== clock) {
    ...
    const handleStartGameWithOptions = (config: { periodType: 'quarters' | 'halves'; clockType: 'running' | 'stop'; periodLength: number; totalPeriods: number; }) => {
        onUpdateGame({
            ...game,
            status: 'live',
            ...config,
            gameClock: config.periodLength, // Set initial clock from config
            currentPeriod: 1, // Ensure period starts at 1
        });
        setClock(config.periodLength); // Sync local clock state
    ...
    if (game.status === 'scheduled') {
        return <GameSetup game={game} onStartGame={handleStartGameWithOptions} onReturnToDashboard={onReturnToDashboard} />;
    }
            ...
            <div className="bg-gray-800 p-4 rounded-lg shadow-2xl mb-2 sticky top-16 z-10">
                <div className="flex justify-between items-center text-center">
                    ...
                    <div className="w-1/3">
                        ...
                        <p className="text-xl md:text-2xl capitalize">{game.periodType?.slice(0, -1) || 'Period'} {game.currentPeriod}</p>
                    </div>
                    <div className="w-1/3">
                ...
                <div className="flex justify-center items-center space-x-2 mt-4 pt-4 border-t border-gray-600 flex-wrap gap-y-2">
                    ...
                    <span className="mx-2 capitalize">{game.periodType?.slice(0, -1) || 'Period'} {game.currentPeriod}</span>
                    <button onClick={() => onUpdateGame({ ...game, currentPeriod: Math.min(game.totalPeriods || 4, game.currentPeriod + 1) })} className="px-3 py-2 bg-gray-600 hover:bg-gray-500 rounded-lg">+</button>

                    <div className="w-px h-6 bg-gray-700 mx-1 hidden sm:block"></div>

                    <button onClick={() => setClock(game.periodLength || 720)} className="px-3 py-2 bg-gray-600 hover:bg-gray-500 rounded-lg">Reset Clock</button>
